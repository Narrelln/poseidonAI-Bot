...
const { analyzeSymbol } = require('./handlers/taClient.js');   // used by /api/ta and TP pulse
...
// === ARES (Adaptive Rescue) SAFE import (won’t crash if missing) ===============
let rescue = null;
try {
  rescue = require('./handlers/rescueManager');
} catch (e) {
  console.log('ℹ️ rescueManager not present yet — skipping ARES wiring for now');
}
// Helpers ARES uses (TA and EM profile)
const { fetchTA } = require('./handlers/taClient');            // TA fetcher
const { getPatternProfile } = require('./handlers/patternStats'); // EM / context
...
app.get('/api/ta/:symbol', async (req, res) => {
  try {
    ...
    app.use('/api', require('./routes/positionsDebugRoute'));
...
// =======================[ ✅ NEW: ARES Rescue Manager wiring ]=======================
(function registerRescueManager() {
  if (!rescue) return; // safe no-op if file not present yet
  const axios = require('axios');

  rescue.setConfig({
    enabled: true,
    hardCutDanger: Number(process.env.ARES_HARD_CUT || 85),
    escapeDanger:  Number(process.env.ARES_ESCAPE || 65),
    dcaMaxX:       Number(process.env.ARES_DCA_MAXX || 2.0),  // total ≤ 2x base
    dcaTpPct:      Number(process.env.ARES_DCA_TP_PCT || 1.6),// TP target after DCA
  });

  rescue.registerExecutors({
    partialClose: async (symbol, qty) => {
      try {
        const exec = require('./handlers/tradeExecutor');
        if (exec && typeof exec.reducePosition === 'function') {
          return await exec.reducePosition({ symbol, quantityContracts: qty, reduceOnly: true });
        }
      } catch {}
      await axios.post(`http://localhost:${PORT}/api/close-trade`, {
        symbol, quantityContracts: qty, reduceOnly: true
      }, { timeout: 12000 });
    },
    closeAll: async (symbol) => {
      try {
        const exec = require('./handlers/tradeExecutor');
        if (exec && typeof exec.closePosition === 'function') {
          return await exec.closePosition({ symbol });
        }
      } catch {}
      await axios.post(`http://localhost:${PORT}/api/close-trade`, {
        symbol, closeAll: true
      }, { timeout: 12000 });
    },
    openAdd: async (payload) => {
      await axios.post(`http://localhost:${PORT}/api/order`, { ...payload, mode: 'ppda' }, { timeout: 12000 });
    },
    emitFeed: (payload) => io.emit('feed', { ...payload, ts: payload?.ts || Date.now() }),
    getTA: fetchTA,
    getPatternProfile,
    // getRails: async (symbol) => (await require('./handlers/extremaRails').getRails(symbol)), // optional
  });

  console.log('🛡️  ARES rescue manager armed');
})();
// ====================[ /NEW: ARES Rescue Manager wiring ]===========================
  ...
  async function pulse() {
    try {
      ...
      for (const p of positions) {
        ...
        // === ARES: drawdown-smart rescue decision (safe if rescue missing) ===
        if (rescue) {
          try {
            await rescue.onTick({
              symbol,
            ...
            console.warn('[ARES] tick error:', e?.message || e);
          }
        }
      }
    } catch (e) {
