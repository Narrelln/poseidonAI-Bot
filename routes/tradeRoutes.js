...
const { toKuCoinContractSymbol } = require('../public/scripts/futuresApiClient');
const { placeFuturesOrder } = require('../handlers/placeTradeHandler'); // ← Make sure this exists!
...
// ✅ New route: Place manual futures trade for testing
router.post('/api/place-futures-trade', async (req, res) => {
  try {
    const { symbol, confidence, note } = req.body;
    ...
    if (!price || price < 0.00001) {
      return res.status(400).json({ success: false, error: `Invalid price for ${symbol}` });
    }

    const notionalUsd = 1;
    const leverage = 5;
    const contractQty = ((notionalUsd * leverage) / price).toFixed(4);
