  ...
  try {
    ...
    data.positions.forEach(pos => {
      ...
      row.innerHTML = `
        ...
        <td>
          <button class="close-btn" onclick="closePosition('${pos.contract}', '${isLong ? 'long' : 'short'}')">✖</button>
        </td>
      `;
...
window.closePosition = async (contract, side) => {
  const button = document.querySelector(`button[onclick*="${contract}"]`);
